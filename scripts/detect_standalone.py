        ...
        def render(g, cam, got):
            ...
            if cand is None:
                ...
                if uv is not None:
                    ...
                    label(out, uv, [
                        ...
                        "foot %.0fx%.0f mm (%.0fx%.0f)%s" % (f.footprint[0] * 1000, f.footprint[1] * 1000, model.dims[0] * 1000, model.dims[1] * 1000, "" if ok_foot else "  X"),
            ...
            if cand is None:
                ...
                lines += ["  margin      %.2f" % cand["margin"],
                          ...
                          "  footprint   %.0f x %.0f mm (%.0f x %.0f, +/-%.0f)" % (f.footprint[0] * 1000, f.footprint[1] * 1000, model.dims[0] * 1000, model.dims[1] * 1000, ds.FOOT_TOL_M * 1000),
                          "  pixels      %d" % f.n_px,
